...
public class DelayedActionHandler : MonoBehaviour {
    private readonly Dictionary<string, Coroutine> _keys = new();
    ...
    private IEnumerator CallAfterSecondsCoroutine(Action<string> action, float seconds, string key) {
    ...
    /// <param name="key">The identifier of the coroutine class</param>
    /// <returns>IEnumerator for coroutine management</returns>
    private IEnumerator CallAfterFixedFramesCoroutine(Action<string> action, int frameCount, string key) {
        while(frameCount > 0) {
    ...
    /// <param name="key">The identifier of the coroutine class</param>
    /// <returns>IEnumerator for coroutine management</returns>
    private IEnumerator CallAtEndOfFrameCoroutine(Action<string> action, string key) {
    ...
    /// <summary>
    /// Stops a currently running coroutine
    /// </summary>
    /// <param name="key">The key of the coroutine</param>
