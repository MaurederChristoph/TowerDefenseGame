...
public class ShootingBehavior : MonoBehaviour {
	...
	private List<Vector3> GetPath(Vector3 origin, Vector3 target, ProjectileInfo projectile) {
		switch(projectile.ShootingType) {
			case ShootingType.Path:
				return projectile.PiercingAmount != 0
					? new List<Vector3> { origin, origin + (target - origin).normalized * 30 }
	...
	private IEnumerator MoveProjectile(GameObject shotProjectile, List<Vector3> path, float speed, ProjectileInfo projectile) {
		var targetIndex = 0;
		if (projectile.PiercingAmount > 0 && projectile.ShootingType != ShootingType.Point) {
			shotProjectile.GetComponent<Bullet>().EnableCollider(projectile.TargetFaction, ReachedDestination, projectile);
		}
		while(shotProjectile != null) {
			var startPoint = path[targetIndex];
			var endPoint = path[targetIndex + 1];
			...
			while(journeyProgress < journeyLength && shotProjectile != null) {
		...
		if ((projectile.PiercingAmount == 0 || projectile.ShootingType == ShootingType.Point) && shotProjectile != null) {
